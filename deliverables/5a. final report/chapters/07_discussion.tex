...
In this project, our main goal was to build a working machine learning model that can find trees in cities. We used \textit{Sentinel-2} satellite images and open tree register data from the cities. During our work and the exploratory data analysis (EDA), we found out some interesting things about the data and also ran into some problems, especially because we did not have perfect ground truth data. 
...
Because of this, our ground truth is not complete. When our model finds a real tree in a private garden, the evaluation script says it is a ``false positive'' and our score goes down. But when we looked at the predictions by eye (for example in Vienna and Barcelona) and compared them with \textit{Google Maps}, we saw that the model is actually right. The \textit{U-Net} learned how a tree looks like from the satellite data and can find trees even if they are not in the city data. So, the 30\% precision is more a problem of the missing data, and not because our model is bad.
...
We also used \textit{OpenStreetMap} (OSM) to get more context and saw big differences between the cities. In Vienna and Paris, a lot of the labeled trees (around 50\% and 40\%) are inside green spaces like parks or forests. But in Hamburg, only 2.6\% are in parks, and almost all of them are street trees. 
